...
import { OutsourcingManagementModel, AddOutsourcingManagementModel } from '../models/outsourcing-management/outsourcing-management.model';
...
export class OutsourceManagementService {
  ...
  addOutsourcing(outsource: AddOutsourcingManagementModel): Observable<OutsourcingManagementModel> {
    let headers = new HttpHeaders();
    headers = headers.set('Content-Type', 'application/json; charset=utf-8').set('Auth-Key', 'liskey');
    const apiUrl = `${ROUTE_CONFIG.ConfigurationServiceBaseUrl}/${this.routePrefix}`;
